...
func (self *Builder) Build() *Archive {
	/*
		...
		which redefines int8 into a function rather than a type
		this means we cannot resolve named types in function signatures yet in this step
		at this step, we can only say that int8 is a function in this package. that's it.

		STEP 3: resolve the symbols
		we don't have custom named types in 0.1, so we don't need to
		resolve named types here
		in fact, we only need to resolve the function signatures
		there will only be types in function signatures, so all symbols required
		should be already resolved at this point
		we can hence build the function calling stack structure, because we
		know the size of each type of the calling args
		we can now also output the binary code here
		in future versions, we need to first resolve the types and consts
